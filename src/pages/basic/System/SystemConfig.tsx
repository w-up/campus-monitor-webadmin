...
export const SystemConfigration = Form.create()(
  observer((props: any) => {
    const { systemConfig } = useStore();

    const { getFieldDecorator, setFieldsValue, getFieldsValue, getFieldValue, validateFields } = props.form;

    const { loading, query, dataSource, handleSearchChange, paginationChange, handleSearchReset, total, saveParam } = systemConfig;
    ...
    return (
      <div className="systemConfigPage">
            ...
            <Row>
              <Form onSubmit={doSubmit}>
                <Table rowKey="id" size="small" bordered pagination={pagination} dataSource={toJS(dataSource)}>
                  <Table.Column title="参数代码" dataIndex="paramCode" key="paramCode" width={200} />
                  <Table.Column title="参数说明" dataIndex="paramIntro" key="paramIntro" width={400} />
                  <Table.Column
                    title="参数值"
                    ...
                    render={(text, record: any, index) => {
                      if (record.paramCode === editableParamCode) {
                        return (
                          <Row>
                            {getFieldDecorator("paramCode", { initialValue: record.paramCode, rules: [{ required: true }] })(<Input hidden placeholder="请输入描述" />)}
                            {getFieldDecorator("paramValue", { initialValue: text, rules: [{ required: true }] })(<Input placeholder="请输入描述" />)}
                          </Row>
                        );
                      } else {
                        return text;
