...
export const DataReplenish = Form.create()(
  observer(({ form, history }: any) => {
        ...
        <Spin spinning={loading}>
          <div style={{ background: "#fff", marginBottom: 20, border: "1px solid #e8e8e8", borderLeft: 0, borderRight: 0, padding: "20px" }}>
            <Breadcrumb>
              ...
              <Breadcrumb.Item>数据补录</Breadcrumb.Item>
            </Breadcrumb>
          </div>
          <Row gutter={10}>
            <Col span={6}>
                  ...
                  <Col span={12}>
                    <Form onSubmit={doSubmit}>
                      ...
                      <Form.Item colon={false} labelAlign="left" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }} label="补测时间">
                        {getFieldDecorator("collectDate", { initialValue: "", rules: [{ required: true, message: "请填写补测时间" }] })(<DatePicker showTime={{ format: "HH" }} size="small" />)}
                      </Form.Item>

                      {pmList.map((item) => {
                        return (
                          <Form.Item colon={false} key={item.pmCode} labelAlign="left" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }} label={item.pmName}>
                            {getFieldDecorator(`pmList[${item.pmCode}]`, { initialValue: "", rules: [{ required: false }] })(<Input size="small" suffix={item.pmUnit} />)}
                          </Form.Item>
                        );
                      })}
                      ...
                      <Form.Item colon={false} labelAlign="left" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }} label="操作说明">
                        {getFieldDecorator("note", { initialValue: "", rules: [{ required: false }] })(<Input.TextArea />)}
                      </Form.Item>
                      ...
                      <Form.Item colon={false} labelAlign="left" labelCol={{ span: 6 }} wrapperCol={{ span: 18 }} label="上传附件">
                        ...
                        <form id="formElem">
                          <div style={{ display: "flex", alignItems: "center" }}>
                            <button
                              style={{ height: "25px", padding: "0 20px", lineHeight: "25px", background: "white", border: "1px solid #ccc" }}
                              onClick={(e) => {
                                e.preventDefault();
