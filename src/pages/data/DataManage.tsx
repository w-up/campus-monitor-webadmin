...
export const DataManagePage = Form.create()(
    ...
    const columns = [
      ...
      },
      {
        ...
        render: (val) => {
          if (val) {
            return (
              <a download="" href={`${globalConfig.apiEndpoint}${val}`} target="_blank">
                下载
              </a>
            );
          }
              ...
              <Card size="small" title="数据查询">
                <Form onSubmit={doSubmit}>
                  ...
                  <Form.Item colon={false} labelAlign="left" labelCol={{ span: 10 }} wrapperCol={{ span: 14 }} label="补录原因">
                    {getFieldDecorator("reason", { initialValue: query.reason, rules: [{ required: false }] })(<Input.TextArea placeholder="请填写补录原因" />)}
                  </Form.Item>
