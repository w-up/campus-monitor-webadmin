...
export class EnterpriseOutScreenMapStore {
  ...
  async loadHoursSewage() {
    ...
    let HoursSewage: EnterpriseOutScreenMapStore["HoursSewage"] = {
  ...
  async loadDailySewage() {
    ...
    let dailySewage: EnterpriseOutScreenMapStore["dailySewage"] = {
      pms: _.get(result, "data.pms", []) || [],
      dates: [],
    };
  ...
  @action.bound
  updateMap(data: Partial<EnterpriseOutScreenMapStore> = {}) {
    if (!this.map) return;
    if (data) {
