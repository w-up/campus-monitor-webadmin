...
export const AlertItem = ({ item, onUpdate, type }: { item: AlarmInfo; onUpdate?: Function; type?: string }) => {
      ...
      <Row className="alertCardRow border-b py-2" style={{ borderColor: "#383F52", lineHeight: "32px" }}>
        ...
        <Col className="text-left" span={6} offset={2}>
          <Button
            ...
            onClick={async () => {
              await mapMonitor.doConfirmAlarmInfoById(type == "popup" ? { alarmId: item.id, isFromPopup: 1 } : { alarmId: item.id });
              onUpdate && onUpdate();
            }}
          >
            处理
